...
const designs = () => {
    ...
    const sliderCounterContentCurrent = '#designs-counter .slider-counter-content__current';
    const sliderCounterContentTotal = document.querySelector('#designs-counter .slider-counter-content__total');
    ...
    sliderUniversal(`.designs-slider__style1`, designRight, designLeft, sliderCounterContentCurrent);
    ...
    const getCurrentSlide = (targetTab) => {
        designsSliderArray.forEach((elem, index, array) => {
            ...
            if (designsListArray.indexOf(targetTab) === index) {
                ...
                document.querySelector('#designs-counter .slider-counter-content__current').textContent = 1;
                array[index].style.display = 'block';
                sliderUniversal(`.${array[index].className}`, designRight, designLeft, sliderCounterContentCurrent);
    ...
    const getCurrentDesignImg = (targetTab) => {
        ...
        designsSliderArray.forEach((elem) => {
            if ((elem.style.display === 'block' && elem.style.display !== 'none') ||
                (elem.className === 'designs-slider__style1' && !elem.hasAttribute('style'))) {
    ...
    designs.addEventListener('click', (event) => {
        ...
        if (target.closest('#designs-list') && target.matches('.designs-nav__item_base')) {
            designsListArray.forEach(elem => {
                elem.classList.remove('active');
            });
            target.classList.add('active');
            getCurrentSlide(target);
        } else if (target.closest('#designs') && target.matches('.preview-block__item-inner')) {
