...
const sendForm = () => {
    ...
    //     if (target.matches('.form-btn')) {
    //         const parentTarget = target.parentNode;
    //         const policy = parentTarget.querySelector('.checkbox__input');
    //         if (!policy.checked) {
    ...
    const postData = (body) => { //Возвращает запрос на сервер
    ...
    forms.forEach((form) => {
        ...
        form.addEventListener('submit', (event) => {
            ...
            if (!policy.checked) {//**Проверка на чекбокс */
                form.appendChild(statusMessage);
                statusMessage.style.cssText = `font-size: 14px;
    color:red`;
                statusMessage.textContent = warnCheck;
            } else if (phone.value.length < 18) {//**Проверка на корректность введённого номера */
                form.appendChild(statusMessage);
                statusMessage.style.cssText = `font-size: 14px;
    color:red`;
