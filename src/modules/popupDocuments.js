...
const popupDocuments = () => {
    ...
    const sliderCounterContentCurrent = '#transparency-popup-counter .slider-counter-content__current';
    ...
    document.querySelector('body').addEventListener('click', (event) => {
        ...
        if (target.matches('.transparency-item__img')) {
            document.querySelector('#transparency-popup-counter .slider-counter-content__current').textContent = 1;
            sliderCounterContentTotal.textContent = document.querySelector('.popup-transparency-slider').children.length;
            popupTransparency.style.visibility = 'visible';
            document.querySelector('body').style.overflow = 'hidden';
        } else if (target.matches('.close') && target.closest('.popup-transparency')) {
            popupTransparency.removeAttribute('style');
            document.querySelector('body').removeAttribute('style');
        }
    });

    sliderUniversal(popupTransparencySlider, transparencyRight, transparencyLeft, sliderCounterContentCurrent);
};
